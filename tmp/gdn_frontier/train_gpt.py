...
class Hyperparameters:
    ...
    post_eval_rank0_only = bool(int(os.environ.get("POST_EVAL_RANK0_ONLY", "1")))
    exit_after_ema_save = bool(int(os.environ.get("EXIT_AFTER_EMA_SAVE", "0")))
...
def main():
    ...
    if "SWA_WINDOW" in os.environ:
        config["swa_window"] = int(os.environ["SWA_WINDOW"])
    if "BIGRAM_VOCAB_SIZE" in os.environ:
        config["bigram_vocab_size"] = int(os.environ["BIGRAM_VOCAB_SIZE"])
    if "BIGRAM_DIM" in os.environ:
        config["bigram_dim"] = int(os.environ["BIGRAM_DIM"])
    if "TRIGRAM" in os.environ:
        config["trigram"] = bool(int(os.environ["TRIGRAM"]))
    if "MLP_MULT" in os.environ:
        config["mlp_mult"] = float(os.environ["MLP_MULT"])
    if "LAYER_LAYOUT" in os.environ:
        config["layer_layout"] = os.environ["LAYER_LAYOUT"]
    ...
    # Local experimentation path: keep training distributed, but perform the
    # expensive EMA/quantization/eval tail on rank 0 only to avoid NCCL stalls.
    if args.post_eval_rank0_only and distributed:
        if not master_process:
            dist.destroy_process_group()
            return
        model = base_model
    ...
    ema_model_path = os.path.join(args.ckpt_dir, f"final_model_{config['arch_name']}_seed{args.seed}.pt")
    if master_process:
        torch.save(base_model.state_dict(), ema_model_path)
        log0("Saved raw EMA model")
        if args.exit_after_ema_save:
            log0("EXIT_AFTER_EMA_SAVE=1, stopping before post-train eval/quantization")
    if args.exit_after_ema_save:
        if distributed:
            dist.destroy_process_group()
        return
