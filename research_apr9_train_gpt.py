...
class ValidationData:
	def __init__(self,h,device):
		...
		if h.val_tokens_limit>0:
			usable=min(h.val_tokens_limit,self.val_tokens.numel()-1);usable=usable//h.eval_seq_len*h.eval_seq_len
			if usable<=0:raise ValueError(f"VAL_TOKENS_LIMIT={h.val_tokens_limit} is too small for EVAL_SEQ_LEN={h.eval_seq_len}")
...
def train_and_eval(h,device):
	...
	if not h.eval_only:
		base_model,compiled_model=train_model(h,device,val_data);torch._dynamo.reset()
		if h.run_prequant_eval:timed_eval('pre-quantization post-ema',eval_val,h,device,val_data,compiled_model)
		serialize(h,base_model,Path(__file__).read_text(encoding='utf-8'))
		if h.distributed:dist.barrier()
	eval_model=deserialize(h,device)
	if h.num_loops>0:eval_model.looping_active=True
	compiled_model=torch.compile(eval_model,dynamic=False,fullgraph=True)
	if h.run_quant_eval:timed_eval('quantized',eval_val,h,device,val_data,compiled_model)
	if h.run_sliding_eval and h.sliding_window_enabled:timed_eval('quantized_sliding_window',eval_val_sliding,h,device,val_data,eval_model)
	if h.run_ttt_eval and h.ttt_enabled and h.sliding_window_enabled:
	...
	if h.run_etlb_eval and h.etlb_enabled and h.sliding_window_enabled:
